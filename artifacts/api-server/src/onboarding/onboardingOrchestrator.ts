...
async function runPhase7(id: string, decidedBy: string) {
  ...
  const prResult = await createGitHubPr(
    agentCard,
    {
      ...
      ...(candidateFiles.exception_authority_md ? { "EXCEPTION_AUTHORITY.md": candidateFiles.exception_authority_md } : {}),
    },
    firstHitlDecidedBy,
    decidedBy,
    evalPassRate
  ...
  await witnessOnboarding("agent_onboarded", {
    ...
    files_created: 3 + (candidateFiles.exception_md ? 1 : 0) + (candidateFiles.exception_authority_md ? 1 : 0),
