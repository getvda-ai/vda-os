  ...
  const handleGenerate = async () => {
    ...
    try {
      const r = await fetch("/api/admin/generate-soc2-sd", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ companyId, triggeredBy: triggeredBy || "Platform User" }),
      });
      const data = await r.json();
      ...
      if (onSaveToWitness) {
        onSaveToWitness({
          ...
          reasoning: `Generated by ${triggeredBy || "Platform User"} — ${data.wordCount} words covering CC6/CC7/CC8/CC9 Trust Service Criteria.`,
        });
      }
      await fetchStatus();
    } catch (e) {
                  ...
                  return (
                    <div key={entry.id} style={{ borderBottom: i < status.recentWitnessEntries.length - 1 ? `1px solid ${T.border}` : "none", padding: "10px 0" }}>
                      ...
                      <div style={{ fontSize: 11, color: T.dim, fontFamily: T.mono, lineHeight: 1.5 }}>
                        {apaleoData.triggeredBy && <div>By: <span style={{ color: T.text }}>{apaleoData.triggeredBy}</span></div>}
